...
int main(void) {
	...
	for (ll q = 0; q < M; q++) {
		...
		if (ms.first >= dur) {
			for (ll i = dur; i < N && ms.second >= i - dur; i++) {
				ll sum = Combination(i - 1, dur - 1);
				sum *= Factorial(ms.first, dur);
				sum *= Combination(ms.second, i - dur);
				sum *= Factorial(N - i, N - i);

				sum *= ((ms.first - dur) * inv[ms.first] * msSum.first
					+ (ms.second - i + dur) * inv[ms.second] * msSum.second);
				ans += sum;
			}
			ans = (ans * rf[N]) % MOD;
		}
